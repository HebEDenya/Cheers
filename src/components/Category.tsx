...
const Category : React.FC<props>= ({user_id}) => {
  ...
  const handleDeletCategory = () => {
    if (deleteCat) {
      axios.delete(`/api/deleteCategory/${deleteCat}`).then((result) => {
        console.log(result);
    ...
    &nbsp;
        ...
        <IonItem className="input_create_categories">
         ...
         &nbsp;
         <IonButton  size="default"  type="submit"  color="warning"  className="category_admin_btn"  expand="full"
         onClick={()=> { if (verif()){postCategory(); setSpiner(true)}
         else if (!verif()){ present('All mandatory * fields must be filled', [{ text: 'Ok' }]) }  }}>Confirm</IonButton>
         <IonLoading
         isOpen= {spiner}
         message='Loading ... 🕓'
         ...
         ></IonLoading>
         &nbsp;
          <IonLabel className="category_admin_delete_text_style"> To delete a category</IonLabel>
          <IonItem className="admin_category">
         <IonLabel position="floating" className="color_subtitle_categories">Category Name</IonLabel>
          <IonInput type="text" name="title" value={deleteCat} onIonChange={e => {setDelete(e.detail.value!);}} 
          clearInput required spellcheck  maxlength= {50} > 
         </IonInput>
         </IonItem>
         <IonButton  size="default"  type="submit"  color="danger"  className="delete_category_admin_btn"  expand="full"
         onClick={()=> {if(deleteCat) { handleDeletCategory(); setSpiner(true)} else {
