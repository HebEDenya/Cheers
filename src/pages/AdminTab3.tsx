...
const AdminTab3: React.FC<adminProps> = () => {
  ...
  useEffect(() => {
    ...
    if (type_user && +userId !== removeButton.btn_Id) {
      setAdminList(
        adminList.filter((item) => item.user_id !== removeButton.btn_Id)
      );
      if (removeButton.btn_Id && +userId !== removeButton.btn_Id) {
        axios
          .delete(`/api/removeAdmin/${removeButton.btn_Id}`)
          .then((result) => {
  ...
  return (
    <IonPage>
      ...
      <IonContent className="admin_dashbord_space">
                ...
                <IonItem className="admin_liste_style" lines="none" key={i}>
                  <IonLabel>
                    <h2 className="text_admin_label_color">
                      {admin.type_user === "superAdmin"
                        ? "Head Admin : " + admin.username
                        : admin.type_user + " : " + admin.username}
                    </h2>
                  </IonLabel>{" "}
                  <IonIcon
                    icon={remove}
                    color="warning"
                    onClick={() => {
                      setremoveButton({ clicked: true, btn_Id: admin.user_id });
                    }}
                  />
                </IonItem>
              );
        ...
        <IonButton
          color="warning"
          className="btn_admin_tab3_style"
          onClick={() => {
            setAddButton(true);
          }}
        >
        ...
        {addButton ? (
          <IonCard>
            <IonCardHeader>
              <IonCardTitle className="add_admintab3_title_style">
                Add a new admin{" "}
              </IonCardTitle>
            </IonCardHeader>
            <IonCardContent>
              <IonItem>
                ...
                <IonInput
                  type="email"
                  value={adminEmail}
                  onIonChange={(e) => setAdminEmail(e.detail.value!)}
                  clearInput
                ></IonInput>
              </IonItem>
              <IonItem>
                ...
                <IonInput
                  type="text"
                  value={adminUsername}
                  onIonChange={(e) => setAdminUsername(e.detail.value!)}
                  clearInput
                ></IonInput>
              </IonItem>
              <IonItem>
