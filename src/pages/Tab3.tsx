...
const Tab3: React.FC<ContainerProps> = ({
  user_id,
  setviewEvent,
  viewEvent,
  setPath,
  ...
  const [heartButtonClick, setHeartButtonClick] = useState<{
    clicked: boolean;
    btn_Id: number | null;
  ...
  useEffect(() => {
    if (user_id) {
      setFavoriteEvent(
        favoriteEvent.filter(
          (item) => item.event_id !== heartButtonClick.btn_Id
        )
      );
      if (heartButtonClick.btn_Id) {
        axios
          .delete(
            `/api/removefromfavorite/${heartButtonClick.btn_Id}/${user_id}`
          )
          .then((result) => {
          ...
          {favoriteEvent.length ? (
                    ...
                    <IonGrid>
                      ...
                      <IonRow>
                        <IonDatetime
                          ...
                          value={moment(item.start_time).format("MMM D YYYY")}
                        ...
                        <IonCol>
                          <IonIcon
                            onClick={() => {
                              ...
                              setTest(item.event_id);
                            }}
                            icon={heart}
                            id="heart_favorite-hover"
                          />
                        </IonCol>
                      </IonRow>
                    </IonGrid>
            ...
            <>
              ...
              <IonContent fullscreen>
                ...
                <IonCard>
                  <img
                    src="https://i.gifer.com/r8e.gif"
                    alt=""
                    className="favorite_img_size"
                  />
                  <IonCardHeader>
                    <IonLabel className="no_title_fav">
                      No Favorites Yet!
                    </IonLabel>
                    <IonButton fill="outline" expand="full" routerLink="/tab1">
                      {" "}
                      Check some events{" "}
                    </IonButton>
                  </IonCardHeader>
